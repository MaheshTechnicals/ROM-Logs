...
# Display the script author and version with stylish headers
echo -e "\e[1;35m###############################################\e[0m"
echo -e "\e[1;36m###          Script by Mahesh Technicals          ###\e[0m"
echo -e "\e[1;33m###             Version 1.3 - Enhanced UI          ###\e[0m"
echo -e "\e[1;35m###############################################\e[0m"

# Function to check if jq is installed
check_dependencies() {
  if ! command -v jq &> /dev/null; then
    echo -e "\e[1;31m⚠ jq is not installed. Installing jq...\e[0m"
    sudo apt-get update
    sudo apt-get install -y jq
  else
    echo -e "\e[1;32m✔ jq is already installed.\e[0m"
  fi
}

# Function to handle script interruption (CTRL+C)
handle_interrupt() {
  echo -e "\n\e[1;31m❌ Script interrupted! Closing SSH session...\e[0m"
  end_ssh_session
  exit 1
}
...
if [ ! -f private.json ]; then
  echo -e "\e[1;31mError: private.json not found! Please provide the file.\e[0m"
  exit 1
fi
...
if [ -z "$SOURCEFORGE_USERNAME" ] || [ -z "$PROJECT_NAME" ]; then
  echo -e "\e[1;31mError: Missing required fields in private.json!\e[0m"
  exit 1
fi
...
if [ ${#FILES[@]} -eq 0 ]; then
  echo -e "\e[1;31mNo .img or .zip files found to upload. Please add files.\e[0m"
  end_ssh_session
  exit 1
fi
...
upload_file() {
  local file=$1
  echo -e "\e[1;34mUploading $file to $UPLOAD_PATH...\e[0m"
  ...
  if [ $? -eq 0 ]; then
    echo -e "\e[1;32m✔ Successfully uploaded $file.\e[0m"
  else
    echo -e "\e[1;31m❌ Failed to upload $file.\e[0m"
  fi
}
...
for number in "${selected_numbers[@]}"; do
  if [ "$number" -eq 1 ]; then
    ...
    echo -e "\e[1;34mPlease enter the full path of the file to upload (auto-completion enabled):\e[0m"
    read -e -p "File path: " custom_file
    if [ -f "$custom_file" ]; then
      upload_file "$custom_file"
    else
      echo -e "\e[1;31m❌ Invalid file path: $custom_file\e[0m"
    fi
  elif [ "$number" -gt 2 ] && [ "$number" -le $(( ${#FILES[@]} + 2 )) ]; then
    # Upload the specific file
    upload_file "${FILES[$((number-3))]}"
  else
    echo -e "\e[1;31m❌ Invalid selection: $number. Please choose a valid number.\e[0m"
  fi
done
...
# Display end message
echo -e "\e[1;35m###############################################\e[0m"
echo -e "\e[1;36m###          Script by Mahesh Technicals - Completed          ###\e[0m"
echo -e "\e[1;35m###############################################\e[0m"
